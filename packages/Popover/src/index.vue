<template>
  <div class="odos-popover" ref="referenceRef">
    ...
    <Teleport to="body">
      <div
        v-show="isVisible || _isVisible"
        ref="floatingRef"
        class="odos-popover-content"
        :class="{ 'popover-show': isVisible || _isVisible }"
...
const addScrollListeners = () => {
  ...
  const windowScrollHandler = () => {
    if (isVisible.value || _isVisible.value) {
      update()
    }
  }
...
const handleClickOutside = (event: MouseEvent) => {
  if (props.trigger === 'click' && (isVisible.value || _isVisible.value)) {
...
watch(
  ...
  async () => {
    if (props.modelValue !== undefined) {
      _isVisible.value = props.modelValue
    }
  }
)
